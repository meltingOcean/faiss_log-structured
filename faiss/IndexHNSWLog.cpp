...
void hnsw_add_vertices(
        IndexHNSWLog& index_hnsw_log,
        size_t n0,
        size_t n,
        const float* x,
        bool verbose,
        bool preset_levels = false) {
    size_t d = index_hnsw_log.d;
    HNSW& hnsw = index_hnsw_log.hnsw;
    size_t ntotal = n0 + n;
    ...
    idx_t check_period = InterruptCallback::get_period_hint(
            max_level * index_hnsw_log.d * hnsw.efConstruction);
...
#pragma omp parallel if (i1 > i0 + 100)
            {
                ...
                std::unique_ptr<DistanceComputer> dis(
                        storage_distance_computer(index_hnsw_log.storage));
                int prev_display =
...
/**************************************************************
 * IndexHNSWLog implementation
 **************************************************************/

IndexHNSWLog::IndexHNSWLog(int d, int M, MetricType metric)
        : Index(d, metric), hnsw(M) {}

IndexHNSWLog::IndexHNSWLog(Index* storage, int M)
        : Index(storage->d, storage->metric_type), hnsw(M), storage(storage) {}

IndexHNSWLog::~IndexHNSWLog() {
    if (own_fields) {
        delete storage;
    }
}

void IndexHNSWLog::train(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexHNSWLogFlat (or variants) instead of IndexHNSWLog directly");
...
#pragma omp for reduction(+ : n1, n2, ndis) schedule(guided)
    ...
    hnsw_stats_log.combine({n1, n2, ndis});
}

} // anonymous namespace

void IndexHNSWLog::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
...
void IndexHNSWLog::range_search(
        idx_t n,
        const float* x,
        float radius,
...
void IndexHNSWLog::add(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexHNSWLogFlat (or variants) instead of IndexHNSWLog directly");
...
void IndexHNSWLog::reset() {
    hnsw.reset();
    storage->reset();
    ntotal = 0;
}

void IndexHNSWLog::reconstruct(idx_t key, float* recons) const {
    storage->reconstruct(key, recons);
}

void IndexHNSWLog::shrink_level_0_neighbors(int new_size) {
#pragma omp parallel
    {
        std::unique_ptr<DistanceComputer> dis(
...
void IndexHNSWLog::search_level_0(
        idx_t n,
        const float* x,
        idx_t k,
...
#pragma omp critical
        { hnsw_stats_log.combine(search_stats); }
    }
}

void IndexHNSWLog::init_level_0_from_knngraph(
        int k,
        const float* D,
        const idx_t* I) {
...
void IndexHNSWLog::init_level_0_from_entry_points(
        int n,
...
void IndexHNSWLog::reorder_links() {
...
void IndexHNSWLog::link_singletons() {
...
void IndexHNSWLog::permute_entries(const idx_t* perm) {
...
/**************************************************************
 * IndexHNSWLogFlat implementation
 **************************************************************/

IndexHNSWLogFlat::IndexHNSWLogFlat() {
    is_trained = true;
}

IndexHNSWLogFlat::IndexHNSWLogFlat(int d, int M, MetricType metric)
        : IndexHNSWLog(
...
/**************************************************************
 * IndexHNSWLogPQ implementation
 **************************************************************/

IndexHNSWLogPQ::IndexHNSWLogPQ() = default;

IndexHNSWLogPQ::IndexHNSWLogPQ(int d, int pq_m, int M, int pq_nbits)
        : IndexHNSWLog(new IndexPQ(d, pq_m, pq_nbits), M) {
    own_fields = true;
    is_trained = false;
}

void IndexHNSWLogPQ::train(idx_t n, const float* x) {
    IndexHNSWLog::train(n, x);
    (dynamic_cast<IndexPQ*>(storage))->pq.compute_sdc_table();
}

/**************************************************************
 * IndexHNSWLogSQ implementation
 **************************************************************/

IndexHNSWLogSQ::IndexHNSWLogSQ(
        int d,
        ScalarQuantizer::QuantizerType qtype,
        int M,
        MetricType metric)
        : IndexHNSWLog(new IndexScalarQuantizer(d, qtype, metric), M) {
    is_trained = this->storage->is_trained;
    own_fields = true;
}

IndexHNSWLogSQ::IndexHNSWLogSQ() = default;

/**************************************************************
 * IndexHNSWLog2Level implementation
 **************************************************************/

IndexHNSWLog2Level::IndexHNSWLog2Level(
        Index* quantizer,
        size_t nlist,
        int m_pq,
        int M)
        : IndexHNSWLog(new Index2Layer(quantizer, nlist, m_pq), M) {
    own_fields = true;
    is_trained = false;
}

IndexHNSWLog2Level::IndexHNSWLog2Level() = default;
...
void IndexHNSWLog2Level::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
    ...
    if (dynamic_cast<const Index2Layer*>(storage)) {
        IndexHNSWLog::search(n, x, k, distances, labels);
...
void IndexHNSWLog2Level::flip_to_ivf() {
